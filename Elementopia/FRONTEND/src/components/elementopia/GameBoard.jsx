...
import { ELEMENTS, matchCompound, explainFailure, shuffle, liveCommentary } from "@/lib/game-data";
...
export function GameBoard({ nickname, domain, onCleared, onExit, onError }) {
  ...
  const synthesize = () => {
    ...
    const matched = matchCompound(workbench, domain);
    if (matched && !solved.includes(matched.formula)) {
      const newSolved = [...solved, matched.formula];
      const newCorrect = correct + 1;
      setSolved(newSolved);
      setCorrect(newCorrect);
      setSynthLog(l => [`✓ ${matched.formula} — ${matched.name}`, ...l].slice(0, 12));
      setWorkbench({});
      setByproduct(null);
      setGlow(true);
    ...
    const msg = matched ? `${matched.formula} already synthesized. Try the next target.` : explainFailure(workbench, domain);
    setByproduct(msg);
    setShake(true);
