...
import { getStoredNickname, setStoredNickname } from '@/lib/session';
import { joinRoom } from '@/lib/room';

export default function ChallengeLobby() {
  const navigate = useNavigate();
  const [nick] = useState(getStoredNickname());
      ...
      <main className="max-w-3xl mx-auto px-6 py-12 w-full">
        ...
        <div className="card-glass rounded-xl p-5 mt-8 border border-magenta/20 shadow-[0_0_15px_rgba(236,72,153,0.15)]">
          ...
          <input 
            value={nick} 
            readOnly
            className="w-full bg-black/40 border border-white/15 rounded-lg px-4 py-3 text-white/50 font-mono outline-none cursor-not-allowed" 
          />
        </div>
